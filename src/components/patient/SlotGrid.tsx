...
export function SlotGrid({
    ...
    return (
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2.5 py-4">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="h-10 rounded-[var(--radius-md)] bg-[hsl(var(--muted)/0.5)] animate-pulse"
          />
        ))}
      </div>
    );
  ...
  if (slots.length === 0) {
    return (
      <div className="rounded-[var(--radius-lg)] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-8 text-center my-4">
        <AlertCircle className="h-8 w-8 text-[hsl(var(--warning))] mx-auto mb-2" />
        <h4 className="font-semibold text-sm text-[hsl(var(--foreground))]">No Available Slots</h4>
  ...
  const renderSlotGroup = (title: string, groupSlots: TimeSlot[], icon: React.ReactNode) => {
    ...
    return (
      <div className="space-y-2 mb-4">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">
          {icon}
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2.5">
          {groupSlots.map((slot) => {
            ...
            let buttonClass = "border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] hover:border-[hsl(var(--primary))] hover:bg-[hsl(var(--primary-light))]";

            if (isSelected) {
              buttonClass = "bg-[hsl(var(--primary))] text-white border-[hsl(var(--primary))] font-bold shadow-[var(--shadow-sm)] scale-[1.02]";
            } else if (slot.status === "BOOKED") {
              buttonClass = "bg-[hsl(var(--muted)/0.3)] text-[hsl(var(--muted-foreground)/0.5)] border-transparent cursor-not-allowed line-through";
            } else if (slot.status === "BLOCKED") {
              buttonClass = "bg-[hsl(var(--muted)/0.2)] text-[hsl(var(--muted-foreground)/0.4)] border-transparent cursor-not-allowed";
            }
  ...
  return (
    <div className="space-y-4">
      {/* Legend */}
      <div className="flex items-center gap-4 text-xs text-[hsl(var(--muted-foreground))] py-2 border-b border-[hsl(var(--border))]">
        ...
        <div className="flex items-center gap-1.5">
          <span className="h-3 w-3 rounded-full bg-[hsl(var(--muted)/0.4)]" />
